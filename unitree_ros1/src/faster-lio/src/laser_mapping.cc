...
bool LaserMapping::LoadParams(ros::NodeHandle &nh) {
    ...
    if (lidar_type == 1) {
        ...
        LOG(INFO) << "Using Hesai xt-16 Lidar";
    } else {
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    // Print them out:
    ROS_INFO_STREAM("LiDAR topic set to: " << lidar_topic);
    ROS_INFO_STREAM("IMU  topic set to: " << imu_topic);
...
void LaserMapping::StandardPCLCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    Timer::Evaluate(
        [&, this]() {
            ...
            // use ROS wall‐clock time, not the LiDAR’s old timestamp:
            double now = ros::Time::now().toSec();
            if (now < last_timestamp_lidar_) {
            ...
            time_buffer_.push_back(now);
            last_timestamp_lidar_ = now;
        },
        "Preprocess (Standard)");
    mtx_buffer_.unlock();
}
...
void LaserMapping::LivoxPCLCallBack(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
    ...
    Timer::Evaluate(
        [&, this]() {
            ...
            // 1) grab the current ROS time
            double now = ros::Time::now().toSec();

            // 2) clear if this “now” went backwards
            if (now < last_timestamp_lidar_) {
            ...
            // 3) your existing sync logic (if you still need it)
            if (!time_sync_en_ && !imu_buffer_.empty() &&
                fabs(last_timestamp_imu_ - now) > 10.0 && !lidar_buffer_.empty()) {
                LOG(INFO) << "IMU and LiDAR not Synced, IMU time: " 
                          << last_timestamp_imu_ << ", LiDAR now: " << now;
            }
            if (time_sync_en_ && !timediff_set_flg_ &&
                fabs(now - last_timestamp_imu_) > 1.0 && !imu_buffer_.empty()) {
                timediff_set_flg_ = true;
                timediff_lidar_wrt_imu_ = now + 0.1 - last_timestamp_imu_;
            ...
            // 5) store the ROS‐time stamp, not msg->header.stamp
            time_buffer_.emplace_back(now);
        },
        "Preprocess (Livox)");
    mtx_buffer_.unlock();
}
