...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        sub_controller_th = threading.Thread(target=self.SubControllerThread)
        
        sub_joint_th.daemon = True
        sub_enable_th.daemon = True
        sub_controller_th.daemon = True
        
        sub_joint_th.start()
        sub_enable_th.start()
        sub_controller_th.start()
    ...
    def SubControllerThread(self):
        """unitree go controller key
        
        """
        rospy.Subscriber('/controller/key', UInt16, self.controller_callback, queue_size=1, tcp_nodelay=True)
        rospy.spin()
    ...
    def pos_callback(self, pos_data):
        ...
        if not self.block_ctrl_flag:
            ...
            rospy.loginfo("mode1: %d", pos_data.mode1)
            rospy.loginfo("mode2: %d", pos_data.mode2)
            rospy.loginfo("enable flag: %d", self.GetEnableFlag())
            if(self.GetEnableFlag()):
                ...
                print("Time elapsed for arm pose control:", str(d_time))
    ...
    def controller_callback(self, key):
        pass
